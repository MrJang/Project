...
<c:choose>
	...
	<c:when test="${member.mem_auth=='0' || member.mem_id==board.mem_id}">
		<input type = "button" class = "myButton3" value = "수정" onClick = "window.location ='app_updateArticle?num=${board.bod_num}&pageNum=${param.pageNum}'">
		<input type = "button" class = "myButton3" value = "삭제" onClick = "window.location = 'app_deleteArticle?num=${board.bod_num}&pageNum=${param.pageNum}'">
	</c:when>
