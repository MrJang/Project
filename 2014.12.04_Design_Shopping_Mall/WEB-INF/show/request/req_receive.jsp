				...
				<tr align="center">
					<c:choose>
						<c:when test="${request.sender_mod==1}">
							<td class = "blinkk2" style = "  border-bottom : 2px solid #AAAAAA; border-right : 2px solid #AAAAAA;" onclick="window.location = 'req_info?sender=${request.req_sender}&req_num=${request.req_num}'">
							...
							<c:choose>
								<c:when test="${request.req_category=='1'}">
									<td class = "blinkk2" style = "  border-bottom : 2px solid #AAAAAA; border-right : 2px solid #AAAAAA;" width="10%" align="center" onclick="window.location = 'req_info?sender=${request.req_sender}&req_num=${request.req_num}'">
...
<!-- 								<b>제작완료</b> -->
						...
						<c:when test="${request.sender_mod==0&&request.req_process!=0}">
							<td style = "border-bottom : 2px solid #AAAAAA; border-right : 2px solid #AAAAAA;" onclick="window.location = 'req_info?sender=${request.req_sender}&req_num=${request.req_num}'">
								<b>${request.req_sender}</b>
							</td>
							<td style = "border-bottom : 2px solid #AAAAAA; border-right : 2px solid #AAAAAA;" onclick="window.location = 'req_info?sender=${request.req_sender}&req_num=${request.req_num}'">
								<b>${request.req_date}</b>
							...
							<c:choose>
								<c:when test="${request.req_category=='1'}">
									<td style = "border-bottom : 2px solid #AAAAAA; border-right : 2px solid #AAAAAA;" width="10%" align="center" onclick="window.location = 'req_info?sender=${request.req_sender}&req_num=${request.req_num}'">
										<img src='../upload/sm_${request.req_img}' alt='이미지 로딩 실패' width='100px' height='100px'>
									</td>
								</c:when>
								<c:when test="${request.req_category=='2'}">
	...
	<!-- 								<b>제작완료</b> -->
						...
						<c:when test="${request.req_process==0}">
							<td class = "blinkk3"style = "border-bottom : 2px solid #AAAAAA; border-right : 2px solid #AAAAAA;" onclick="window.location = 'req_info?sender=${request.req_sender}&req_num=${request.req_num}'">
								<b>${request.req_sender}</b>
							</td>
							<td class = "blinkk3"style = "border-bottom : 2px solid #AAAAAA; border-right : 2px solid #AAAAAA;" onclick="window.location = 'req_info?sender=${request.req_sender}&req_num=${request.req_num}'">
								<b>${request.req_date}</b>
							...
							<c:choose>
								<c:when test="${request.req_category=='1'}">
									<td class = "blinkk3"style = "border-bottom : 2px solid #AAAAAA; border-right : 2px solid #AAAAAA;" width="10%" align="center" onclick="window.location = 'req_info?sender=${request.req_sender}&req_num=${request.req_num}'">
										<img src='../upload/sm_${request.req_img}' alt='이미지 로딩 실패' width='100px' height='100px'>
									</td>
								</c:when>
								<c:when test="${request.req_category=='2'}">
	...
	// 		TODO ls
		...
		<table width = "100%" border = "0">
			...
			<c:choose>
				<c:when test="${param.pageNum>5}">
					<span style="text-align: center; font-size: 50px; cursor : pointer;"
						onClick="location.href = 'req_receive?pageNum=${((param.pageNum)-5)}'">◀</span>
				</c:when>
			</c:choose>
			<%
				for (i = (e + pg) - 5; i < j; i++) {
			...
			<span style="text-align: center; font-size: 50px; cursor:pointer;"
				onClick="location.href = 'req_receive?pageNum=<%=i%>'"><%=i%></span>
			<%
				} else {
			%>
			...
			%>
			<span style="text-align: center; font-size: 50px; cursor : pointer;"
				onClick="location.href = 'req_receive?pageNum=<%=e + pg%>'">▶</span>
			<%
				}
			%>
