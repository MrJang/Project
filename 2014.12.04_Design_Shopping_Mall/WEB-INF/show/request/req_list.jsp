	...
	<c:forEach var="request" items="${list}" begin = "${((param.pageNum-1))*5}" end = "${((param.pageNum-1)*5)+4}">
		...
		<tr align = "center">
			<c:choose>
				<c:when test="${request.receiver_mod==1}">
					<td  class = "blinkk2" style = "  border-bottom : 2px solid #AAAAAA; border-right : 2px solid #AAAAAA;" onclick="window.location = 'my_req_info?receiver=${request.req_receiver}&req_num=${request.req_num}'">
		...
		<!-- 						<b>제작완료</b> -->
				...
				<c:when test="${request.receiver_mod==0}">
					<td style = "border-bottom : 2px solid #AAAAAA; border-right : 2px solid #AAAAAA;" onclick="window.location = 'my_req_info?receiver=${request.req_receiver}&req_num=${request.req_num}'">
						<b>${request.req_name}</b>
					</td>
					<td style = "border-bottom : 2px solid #AAAAAA; border-right : 2px solid #AAAAAA;" onclick="window.location = 'my_req_info?receiver=${request.req_receiver}&req_num=${request.req_num}'">
						<b>${request.req_receiver}</b>
...
<%
	// 		TODO ls
		...
		<table width = "100%" border = "0">
			...
			<c:choose>
				<c:when test="${param.pageNum>5}">
					<span style="text-align: center; font-size: 50px; cursor:pointer;"
						onClick="location.href = 'req_list?pageNum=${((param.pageNum)-5)}'">◀</span>
				</c:when>
			</c:choose>
			<%
				for (i = (e + pg) - 5; i < j; i++) {
			...
			<span style="text-align: center; font-size: 50px; cursor : pointer;"
				onClick="location.href = 'req_list?pageNum=<%=i%>'"><%=i%></span>
			<%
				} else {
			%>
			...
			%>
			<span style="text-align: center; font-size: 50px; cursor:pointer;"
				onClick="location.href = 'req_list?pageNum=<%=e + pg%>'">▶</span>
			<%
				}
			%>
...
<%-- 	<center> --%>
<!-- 		<input type = "button" class = "return_list" value = "디자이너 목록으로" onclick = "window.location = 'req_main?selected_designer=none'"> -->
<%-- 	</center> --%>
</form:form>
</body>
</html>