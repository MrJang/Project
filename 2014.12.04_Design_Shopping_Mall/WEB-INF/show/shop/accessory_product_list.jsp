...
<body>
	...
	<div class="product_list">
		<div class="product_list_title"><img src="resources/shop_image/accessory_list_title.png"></div>
		<div class="product_list_search_bar">
		<form name="search_form" method="post" action="select_product?pageNum=1&category_num=3">
		...
		<c:forEach var="product" items="${accessory_list}" begin="${start_row_num}" end="${start_row_num+7}">
	...
	<div class="product_purchase_button" onClick="window.location='product_purchase?pro_num=${product.pro_num}&pageNum=${pageNum}&category_num=${category_num}'">구매하기</div>
		...
		<c:when test="${list_exist=='yes'}">
		 <c:choose>
		  ...
		  <c:when  test="${mode=='nomal'}">	
			<a href='#' onClick="window.location='accessory_product?pageNum=${start_pageNum}&category_num=${category_num}'"><img src="resources/shop_image/start_page_img.png" style="float:left;"></a>
			<c:forEach var="page_list" items="${page_list}">
				<a class="page_num_wrap"  onClick="window.location='accessory_product?pageNum=${page_list}&category_num=${category_num}'">	
					${page_list}
				</a>	
			</c:forEach>	
			<a href='#' onClick="window.location='accessory_product?pageNum=${end_pageNum}&category_num=${category_num}'"><img src="resources/shop_image/end_page_img.png" style="float:left;"></a>
		  </c:when>
		  </c:choose>
		</c:when>
		</c:choose>	
		...
		<div class="button_wrap">
				...
				<a class="product_list_button" onClick="window.location = 'accessory_product?pageNum=1&category_num=3'">목록</a>
		</div>
	</div>
</body>
</html>